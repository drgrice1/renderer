...
BEGIN {
	...
	$MOJO_CONFIG = -r "$RENDER_ROOT/renderer.conf" ? "$RENDER_ROOT/renderer.conf" : "$RENDER_ROOT/renderer.conf.dist";
}
...
sub sanitizeHostURLs {
	...
	if ($baseURL =~ m!^https?://!) {
		...
		$main::basehref = Mojo::URL->new($SITE_HOST)->path("$baseURL/");
	} else {
	...
	if ($formURL =~ m!\S!) {
		...
		$main::formURL = Mojo::URL->new($SITE_HOST)->path("$baseURL/render-api");
	}
